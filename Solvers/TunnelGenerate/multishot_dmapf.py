...
from clingo import SymbolType, TruthValue

# allowed vertices / tunnel added
cur_plans = []
cur_assumptions = []
cur_preferences = []
cur_paths = []
...
def get_paths(sorted_agents, out_plans, makespan, prg):
    ...
    for a in sorted_agents:
        ...
        #first if used for not changing the allowed cells after each replan
    ...
    for agent, vertices in visited_dict.items():
        ...
        for vertex in adj_list:
            if vertex not in allowed_list:
                prg.release_external(Function("tunnel", [Number(agent), Number(vertex)]))

        allowed_dict[agent] = allowed_list
    prg.cleanup()
    return allowed_dict

def add_tunnel_externals(prg):

    with prg.backend() as backend:
        for agent in agent_list:
            for vertex in adj_list:
                    tunnel_atom = Function(name="tunnel", arguments=[agent,Number(vertex)])
                    atm_a = backend.add_atom(tunnel_atom)
                    backend.add_external(atm_a, TruthValue.True_)
...
def on_model(prg):
    ...
    for atom in prg.symbols(atoms=True):
        ...
        if not graph_checked:       
            ...
            if atom.name == "agent":
                agent_list.append(atom.arguments[0])
...
def solve(prg, s):
    ...
    while ((imax is None or step < imax.number) and
        ...
        if step > 0:
            ...
            prg.ground([("base", [])])
            ret = prg.solve(on_model=on_model, assumptions=cur_assumptions)
            add_tunnel_externals(prg)
...
## solve MAPF, we have grounded program for MAPF and initial agents, 
...
while inp != "end":
    ...
    if "%" in inp:
        ...
        tunnels_dict = get_paths(sorted_agents,out_plans,current_makespan,prg)
